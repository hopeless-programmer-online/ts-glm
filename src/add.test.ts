import { add, vec2, vec3, vec4, mat2, mat3, mat4 } from './glm'

it('should add number and number', () => {
    expect(add(1, 2)).toBe(1 + 2)
})
it('should add number and vec2', () => {
    expect(add(5, vec2(1, 2))).toMatchObject({ x : 5 + 1, y : 2 + 5 })
})
it('should add number and vec3', () => {
    expect(add(5, vec3(1, 2, 3))).toMatchObject({ x : 5 + 1, y : 5 + 2, z : 5 + 3 })
})
it('should add number and vec4', () => {
    expect(add(5, vec4(1, 2, 3, 4))).toMatchObject({ x : 5 + 1, y : 5 + 2, z : 5 + 3, w : 5 + 4 })
})
...
it('should add vec2 and number', () => {
    expect(add(vec3(1, 2, 3), 5)).toMatchObject({ x : 1 + 5, y : 2 + 5, z : 3 + 5 })
})
it('should add vec3 and vec2 as vec2', () => {
    const r = add(vec3(1, 2, 3), vec2(4, 5))
...
it('should add vec4 and number', () => {
    expect(add(vec4(1, 2, 3, 4), 5)).toMatchObject({ x : 1 + 5, y : 2 + 5, z : 3 + 5, w : 4 + 5 })
})
it('should add vec4 and vec2 as vec2', () => {
    const r = add(vec4(1, 2, 3, 4), vec2(5, 6))
...
it('should add vec4 and vec3 as vec3', () => {
    const r = add(vec4(1, 2, 3, 4), vec3(5, 6, 7))

    expect(r).toMatchObject({ x : 1 + 5, y : 2 + 6, z : 3 + 7 })
    expect(r).not.toHaveProperty('w')
})
it('should add vec4 and vec4', () => {
    expect(add(vec4(1, 2, 3, 4), vec4(5, 6, 7, 8))).toMatchObject({ x : 1 + 5, y : 2 + 6, z : 3 + 7, w : 4 + 8 })
})
