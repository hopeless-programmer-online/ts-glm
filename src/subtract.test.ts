import { Vector2, Vector3, Vector4, Matrix2x2, Matrix3x3, Matrix4x4, vec2, vec3, vec4, mat2, mat3, mat4, sub } from './glm'

it('should sub number and number', () => {
    const r : number = sub(1, 2)

    expect(r).toBe(1 - 2)
})
it('should sub number and vec2', () => {
    const r : Vector2 = sub(5, vec2(1, 2))

    expect(r).toMatchObject({ x : 5 - 1, y : 5 - 2 })
})
it('should sub number and vec3', () => {
    const r : Vector3 = sub(5, vec3(1, 2, 3))

    expect(r).toMatchObject({ x : 5 - 1, y : 5 - 2, z : 5 - 3 })
})
it('should sub number and vec4', () => {
    const r : Vector4 = sub(5, vec4(1, 2, 3, 4))

    expect(r).toMatchObject({ x : 5 - 1, y : 5 - 2, z : 5 - 3, w : 5 - 4 })
})
it('should sub number and mat2', () => {
    const r : Matrix2x2 = sub(
        20,
        mat2(
            1, 2,
            3, 4,
...
it('should sub number and mat3', () => {
    const r : Matrix3x3 = sub(
        20,
        mat3(
            1, 2, 3,
            4, 5, 6,
...
it('should sub number and mat4', () => {
    const r : Matrix4x4 = sub(
        20,
        mat4(
            1,  2,  3,  4,
            5,  6,  7,  8,
...
it('should sub vec2 and number', () => {
    const r : Vector2 = sub(vec2(1, 2), 5)

    expect(r).toMatchObject({ x : 1 - 5, y : 2 - 5 })
})
it('should sub vec2 and vec2', () => {
    const r : Vector2 = sub(vec2(1, 2), vec2(3, 4))

    expect(r).toMatchObject({ x : 1 - 3, y : 2 - 4 })
})
it('should sub vec2 and vec3 as vec2', () => {
    const r : Vector2 = sub(vec2(1, 2), vec3(3, 4, 100))

    expect(r).toMatchObject({ x : 1 - 3, y : 2 - 4 })
    expect(r).not.toHaveProperty('z')
})
it('should sub vec2 and vec4 as vec2', () => {
    const r : Vector2 = sub(vec2(1, 2), vec4(3, 4, 100, 200))
...
it('should sub vec3 and vec2 as vec2', () => {
    const r : Vector2 = sub(vec3(1, 2, 3), vec2(4, 5))

    expect(r).toMatchObject({ x : 1 - 4, y : 2 - 5 })
    expect(r).not.toHaveProperty('z')
})
it('should sub vec3 and vec3', () => {
    const r : Vector3 = sub(vec3(1, 2, 3), vec3(4, 5, 6))

    expect(r).toMatchObject({ x : 1 - 4, y : 2 - 5, z : 3 - 6 })
})
it('should sub vec3 and vec4 as vec3', () => {
    const r = sub(vec3(1, 2, 3), vec4(4, 5, 6, 7))
...
it('should sub vec4 and vec2 as vec2', () => {
    const r : Vector2 = sub(vec4(1, 2, 3, 4), vec2(5, 6))

    expect(r).toMatchObject({ x : 1 - 5, y : 2 - 6 })
    expect(r).not.toHaveProperty('z')
    expect(r).not.toHaveProperty('w')
})
it('should sub vec4 and vec3 as vec3', () => {
    const r : Vector3 = sub(vec4(1, 2, 3, 4), vec3(5, 6, 7))
